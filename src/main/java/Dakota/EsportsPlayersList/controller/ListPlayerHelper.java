...
public class ListPlayerHelper {
 ...
 // method to search for player by ID
    ...
    //method to search for players by team name
    public List<ListPlayer> searchForPlayerByTeam(String teamName) {
        EntityManager em = emfactory.createEntityManager();
        TypedQuery<ListPlayer> typedQuery = em.createQuery(
                "select lp from ListPlayer lp where lp.team = :teamName", ListPlayer.class);
